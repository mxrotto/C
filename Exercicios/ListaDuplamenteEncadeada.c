...
void sair(cell* lista) {
	...
	if (opc == 's' || opc == 'S') {
		if (lista != NULL) {
			...
			while (lista->prev != NULL) {				//Apagando alocacoes antes de sair do programa
				dump = lista;					//dump recebe o ultimo endereco
				lista = lista->prev;				//lista recebe o endereco anterior
				free(dump);					//antigo endereco da lista apagado
			}
			free(lista);						//ultimo endereco a ser limpo
		}
		printf("saindo...\n");
		exit(EXIT_SUCCESS);
	}
